...
public class AgregarVacuna extends JDialog {
	...
	public AgregarVacuna() {
		...
		JTextArea txtrDescripcion;
		{
			...
			{
				textDosis = new JTextField("0.1");
				textDosis.setBounds(85, 182, 67, 22);
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						float dosis = !textDosis.getText().trim().isEmpty()
								? Float.parseFloat(textDosis.getText().trim())
								: 0f;
						...
						if (nombre.isEmpty() || fabricante.isEmpty() || dosis < 0.1 || descripcion.isEmpty()) {
							if (dosis < 0.1) {
								JOptionPane.showMessageDialog(AgregarVacuna.this, "Dosis debe ser mayor que 0",
										"Alerta", JOptionPane.ERROR_MESSAGE);
							} else {
								JOptionPane.showMessageDialog(AgregarVacuna.this, "Hay Campos Faltantes", "Alerta",
										JOptionPane.ERROR_MESSAGE);
							}
						} else {
							JOptionPane.showMessageDialog(AgregarVacuna.this, "Vacuna Fue Creada", "Informacion",
									JOptionPane.INFORMATION_MESSAGE);
							logico.Vacuna v = new logico.Vacuna(java.util.UUID.randomUUID().toString(), nombre,
									fabricante, dosis, descripcion);
							dao.VacunaDAO.registrarVacuna(v);
							dispose();
						}
